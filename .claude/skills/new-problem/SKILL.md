...
2. **Render the template**: for each `*.in` file under `problems/_template/`, create the
   ...
   Result: `CMakeLists.txt`, `GOALS.md`, `README.md`, `src/main.cpp`, `tests/test_smoke.cpp`.

3. **Configure and build** (the top-level glob only sees the new directory on reconfigure):
   ```sh
   cmake --preset debug
